...
Status: living design notes, updated 2026-08-14. Sections 1–4 describe the economy as **built**
(the bounty layer has shipped since the first draft); sections 5–8 analyze attacks, safe
consumption, what an oracle-grade outcome would take, and the sybil gate; sections 9–10 read the
first live runs against the design. Decisions here were made in design interviews; the
hard-to-reverse core is recorded in [ADR-0008](adr/0008-outcome-is-a-signal-not-an-oracle.md), and
the market redesign of 2026-08-14 in [ADR-0013](adr/0013-time-weighted-tally.md) and
[ADR-0014](adr/0014-tally-settled-redemption.md).
...
The outcome (`descendantsAggregate > 0` at the root; a tie reads as objected — fail-safe, and
since ADR-0012 zero is the resting state of an empty or all-refuted tree, not a knife-edge) is a
...
| **Last-block snipe**                      | Redemption pays the final reserve ratio — the price the attacker set — so pushing a thin market from 50% to ~99% costs only the fee plus rounding. In the last block nobody can correct it, so the outcome is flippable for ~a fee. Centering (ADR-0012) doubles the upward leverage — 2 sway points per approval point — while bounding downward snipes at silence                                                                                                                                                                                                                                                                               | **Dead by design once ADR-0013/0014 ship**: the tally reads time-weighted approvals and weights, and redemption settles at the weighted rating, so a last-block push moves both the outcome and every payout by ~the tail fraction of the window — moving them materially requires holding a mispriced market all window, feeding correctors. Live deployments (which are immutable) retain the vector until then                                                            |
...
| **Weight stuffing via self-rated decoys** | Stake is voice (ADR-0011), and volume at the price extremes round-trips nearly free: attach a garbage argument to the side you _oppose_, then buy the side that redeems near 1 (crashing your own decoy) — fees return to you as its creator, principal redeems at ~face value, net cost ≈ the 10-token deposit. The decoy's ~100-token subtree weight dilutes its genuinely strong siblings' shares, and correctly down-rating it fixes the _price_ while _feeding the weight_ — correction cannot remove stake | Closed to a residual by ADR-0013: weight comes from stake held through the window, so stuffed volume pays a full window of locked, crashable exposure for the voice it used to get free — while deposits, placed in editing, keep full weight. Residual (from ADR-0014): planting supportive children under an argument one holds shares in lifts its rating and thus one's own settlement — bounded by the same window exposure and by the clamp (a crashed plant folds at zero sway and its weight drags the parent toward neutral). Any mechanism that attracts stake to chosen arguments (e.g. per-argument bounties, §11) industrializes this vector |
| Straw-man pump                            | Plant a weak attack on the thesis you support, then refute it hard: under the pre-ADR-0012 fold the refuted attack's negative blend, negated by its con stance, *confirmed* the thesis — observed live (debate 4, argument 7: an attack at −55.5% blend swaying its thesis +5.6%), and it out-paid arguing for the thesis directly                                                                                                                                                                                       | Dead by design since ADR-0012: a refuted argument is clamped to zero sway, so the pump yields exactly nothing — the deposit and the refuters' stakes buy silence, same as not planting it                                                                                                                                                                                                                                       |
...
| Thesis framing                            | Negated or loaded thesis makes the boolean mean the opposite of what a consumer assumes                                                                                                                                                                                                                                                                                                                                                                                                                          | Social layer; a proposal names the debate id, which is unique and immutable, and quotes the thesis the consumer vetted (its text is in the `DebateCreated` log)                                                                                                                                                                                                                                                                                                                                                         |
...
1. **Reference the debate by its id** and quote its thesis in the proposal — the id is unique and
   never changes, and the thesis text is in the debate's `DebateCreated` log, nowhere in state
   (ADR-0015): a consumer holds the id of the debate it created or vetted, and a debate about any
   other thesis is a different id.
...
1. **Influence is priced in personhood, not capital.** Points are free and non-purchasable, so the
   ...
   can see.
2. **Settlement is self-referential.** Redemption pays the debate's own verdict; there is no
   ...
   subsidize informed traders who profit _when reality settles the market_. ADR-0014 narrows this
   gap without closing it: a market no longer settles at its own closing price but at the tree's
   weighted rating, so a single market is resolved by everything argued beneath it — yet the tree
   is still endogenous to the debate, and "reality" still never arrives from outside.
...
|---|---|---|---|
| [Subsidized market maker](https://courses.cs.duke.edu/spring17/compsci590.2/market_scoring.pdf) (Hanson's LMSR) | Moving a price toward its final value | Fixed `b·ln(outcomes)` per market | Replace the author's sunk deposit as market seed with bounty-funded reserves at a deliberately uninformative price: every correction toward the weighted rating becomes positive-EV, authors stop being the forced noise traders, and the creator's maximum loss stays bounded as today |
| Per-report [proper scoring](https://arxiv.org/pdf/2101.01816), pool-normalized | Accuracy of each stake against the final weighted rating, paid pro-rata from `R` | `R` exactly | A proportional-prize contest: everyone with a positive score earns something, entry improves (the experimental result above), and the §4 denominator logic survives as normalization. Self-referential: "accuracy" means agreement with the crowd's final price |
...
- **Time-weighted tally — decided (2026-08-14).** ADR-0013: the tally reads time-weighted
  approvals _and_ time-weighted weights, one accumulator each per argument, over the rating
  window. Ships with the next deployment.
- **Tally-settled redemption — decided (2026-08-14).** ADR-0014: a share settles at
  `(1 ± rating)/2` — the tree's verdict, unclamped — instead of the market's closing price;
  identical for childless arguments, subtree-corrected for debated ones. Requires ADR-0013
  (settlement through final prices would widen the snipe from the outcome to every ancestor's
  redemption). Ships with the next deployment.
- **Content in the log — decided (2026-09-02).** ADR-0015: a thesis or argument's text (1 to 256
  bytes of UTF-8) is passed to the creating or altering call and published by its event; nothing
  is stored or content-addressed, and IPFS leaves the stack. Consumers reference a debate by its
  id and quote the thesis (§6). Ships with the next deployment.
